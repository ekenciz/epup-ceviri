...
def build_glossary_prompt(
    ...
    *,
    ...
    for entry in entries:
        ...
        if _normalize_glossary_text(entry.source) == _normalize_glossary_text(entry.target):
            line += " | Preserve this term exactly; do not expand or translate it."
        if entry.note:
...
class GlossaryComplianceError(ValueError):
    """Raised when a model ignored one or more relevant glossary entries."""


def _json_payload_candidates(text: str) -> list[dict]:
    """Extract one or more JSON objects from a model response.

    Some models occasionally emit two JSON objects back-to-back even when the
    prompt asks for one. json.loads() reports this as ``Extra data``.  We keep
    every complete top-level object so translation maps can be recovered and
    merged instead of discarding an otherwise usable response.
    """
    ...
    while pos < length:
        while pos < length and cleaned[pos].isspace():
            pos += 1
        if pos >= length:
            break

        # Skip prose/noise until the next possible JSON object.  This also
        # handles a valid object followed by a short explanatory sentence.
        if cleaned[pos] != "{":
            next_obj = cleaned.find("{", pos + 1)
            if next_obj == -1:
                break
            pos = next_obj
    ...
    if not candidates:
        if first_error is not None:
            raise ValueError(f"Model returned invalid JSON: {first_error}") from first_error
        raise ValueError("Model returned invalid JSON: no JSON object found.")
    return candidates


def _parse_chunk_response(response_text: str, expected_ids: tuple[str, ...]) -> dict[str, str]:
    payloads = _json_payload_candidates(response_text)
    expected = set(expected_ids)

    translation_maps: list[dict[str, str]] = []
    for payload in payloads:
        translations = payload.get("translations")
        if isinstance(translations, dict):
            translation_maps.append(translations)
            continue

        # Some models omit the wrapper and return the segment-ID map directly,
        # e.g. {"s000001":"...","s000002":"..."}. Accept it only when
        # every key is an expected segment ID and every value is a string, so
        # unrelated JSON objects cannot be mistaken for translations.
        if payload and set(payload).issubset(expected) and all(
            isinstance(value, str) for value in payload.values()
        ):
            translation_maps.append(payload)

    if not translation_maps:
        raise ValueError("Model response must contain translations for the expected segment IDs.")

    # Prefer any single complete object.  When a model duplicated its answer,
    # this avoids creating artificial conflicts while safely ignoring trailing
    # JSON/prose.
    for translations in translation_maps:
        if set(translations.keys()) == expected:
            for segment_id, value in translations.items():
                if not isinstance(value, str) or not value.strip():
                    raise ValueError(
                        f"Translation for {segment_id} is empty or not a string."
                    )
            return dict(translations)

    # Otherwise merge continuation objects.  The first non-empty value wins;
    # later objects can fill IDs omitted from the first object.
    merged: dict[str, str] = {}
    for translations in translation_maps:
        for segment_id, value in translations.items():
            if segment_id not in merged and isinstance(value, str) and value.strip():
                merged[segment_id] = value
    ...
    for segment_id, value in merged.items():
        if not isinstance(value, str) or not value.strip():
...
def _validate_glossary_compliance(
    ...
    """Validate glossary terms only in segments where their source occurs.

    This is intentionally segment-aware.  A target term appearing elsewhere in
    a large chunk must not hide a real miss, and a glossary entry is not checked
    against unrelated translated segments.
    """
    if not entries:
        return

    missing: list[GlossaryEntry] = []
    for entry in entries:
        # Entries such as SAT=>SAT, Pop=>Pop or The Beatles=>The Beatles are
        # preservation hints. They remain in the prompt, but exact-output
        # validation is intentionally skipped: enforcing them caused expensive
        # retries even when the surrounding translation was otherwise valid.
        if _normalize_glossary_text(entry.source) == _normalize_glossary_text(entry.target):
            continue

        matched_segment_ids = [
            segment.id for segment in source_segments if entry.matches(segment.text)
        ]
        if not matched_segment_ids:
            continue
        expected_target = _normalize_glossary_text(entry.target)
        if not any(
            expected_target in _normalize_glossary_text(translations.get(segment_id, ""))
    ...
    if missing:
        ...
        raise GlossaryComplianceError(f"Glossary terms were not respected: {terms}")
...
def _is_structural_translation_error(exc: Exception) -> bool:
    ...
    markers = (
        ...
        'translations" json object',
        "translations for the expected segment ids",
        "empty response",
...
def _translate_single_segment_plain(
    *,
    provider,
    system_prompt: str,
    segment,
    max_tokens: int,
    temperature: float,
    relevant_entries: tuple[GlossaryEntry, ...],
    enforce_glossary: bool,
    max_retries: int,
    fname: str,
    log_callback=None,
) -> dict[str, str]:
    """Fallback for models that cannot reliably emit JSON for one segment.

    Once recovery has reduced a request to a single segment, JSON buys us no
    structural safety: the segment ID is already known locally. Asking for only
    translated text is substantially more reliable with local and reasoning
    models, so we can reattach the original ID ourselves.
    """
    ...
    prompt = (
        f"You are a professional literary translator. Translate the supplied text "
        f"to the requested target language according to these instructions:\n"
        f"{system_prompt}\n\n"
        "SINGLE-SEGMENT FALLBACK RULES:\n"
        "Return ONLY the translated text itself. No JSON, no quotes, no markdown, "
        "no labels, and no explanation."
    )
    prompt = build_glossary_prompt(prompt, relevant_entries, enforce=enforce_glossary)
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # A few models wrap plain text in one pair of JSON-like quotes.
            if len(text) >= 2 and text[0] == text[-1] == '"':
                try:
                    ...
                    pass
            if not text:
                raise ValueError("Single-segment fallback returned empty text.")

            translations = {segment.id: text}
            if enforce_glossary:
                try:
                    _validate_glossary_compliance(
                        translations, (segment,), relevant_entries
                    )
                except GlossaryComplianceError as glossary_exc:
                    if attempt < max_retries:
                        raise
                    emit(
                        f"⚠️ {fname}: accepting single-segment fallback after glossary "
                        f"retries: {glossary_exc}"
                    )
            return translations
            ...
            if attempt < max_retries:
                emit(
                    f"⚠️ {fname}: single-segment plain fallback attempt "
...
def _translate_chunk(
    *,
    ...
    # JSON overhead grows with segment count even when the text-token budget is
    # small (notes/endnotes often contain hundreds of tiny nodes).  Proactively
    # cap segment count so the model never has to emit enormous translation maps.
    max_segments_per_request = 24
    if (
        len(chunk.segments) > max_segments_per_request
        and recovery_depth < max_recovery_depth
    ):
        children, rejoin_id = _split_chunk_for_recovery(chunk)
        if children:
            emit(
                f"↳ {fname}: chunk {chunk.index + 1} has {len(chunk.segments)} "
                f"segments; pre-splitting before request (recovery depth "
                f"{recovery_depth + 1})."
            )
            merged: dict[str, str] = {}
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if enforce_glossary:
                try:
                    _validate_glossary_compliance(
                        translations, chunk.segments, relevant_entries
                    )
                except GlossaryComplianceError as glossary_exc:
                    # Glossary adherence is a quality constraint, not a reason
                    # to discard an otherwise valid chapter forever. Retry with
                    # stronger instructions, then accept the final valid JSON
                    # with a visible warning if the model still refuses one term.
                    if attempt < max_retries:
                        raise
                    emit(
                        f"⚠️ {fname}: accepting valid translation after glossary "
                        f"retries: {glossary_exc}"
                    )
            return translations
        except Exception as exc:
            ...
            # If only one segment remains, stop asking the model to reproduce
            # JSON structure. Reattach the known segment ID locally and request
            # plain translated text instead. This is the terminal recovery path
            # for repeated missing-ID/wrapper/invalid-JSON failures.
            if structural and len(chunk.segments) == 1:
                emit(
                    f"↳ {fname}: chunk {chunk.index + 1} reached one segment; "
                    "switching to plain-text fallback."
                )
                return _translate_single_segment_plain(
                    provider=provider,
                    system_prompt=system_prompt,
